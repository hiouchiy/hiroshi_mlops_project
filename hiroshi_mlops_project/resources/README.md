...
The databricks CLI bundles top level is defined by file `hiroshi_mlops_project/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`hiroshi_mlops_project/resources/model-workflow-resource.yml`)
 - batch inference workflow (`hiroshi_mlops_project/resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`hiroshi_mlops_project/resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`hiroshi_mlops_project/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`hiroshi_mlops_project/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `hiroshi_mlops_project/databricks.yml` and `hiroshi_mlops_project/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-hiroshi_mlops_project-model     | /dev-hiroshi_mlops_project-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-hiroshi_mlops_project-model | /staging-hiroshi_mlops_project-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-hiroshi_mlops_project-model    | /prod-hiroshi_mlops_project-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-hiroshi_mlops_project-model    | /test-hiroshi_mlops_project-experiment    |
...
2. Complete the `TODO` in `hiroshi_mlops_project/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `hiroshi_mlops_project/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `hiroshi_mlops_project/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-hiroshi_mlops_project-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-hiroshi_mlops_project-batch-inference-job`
that runs the notebook under `hiroshi_mlops_project/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-hiroshi_mlops_project-batch-inference-job`.
...
`test-hiroshi_mlops_project-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-hiroshi_mlops_project-batch-inference-job`.
